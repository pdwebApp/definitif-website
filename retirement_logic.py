# Importing all the necessary libraries
import pandas as pd
import numpy as np
import numpy_financial as npf
import plotly.express as px

# Import time related 
from dateutil import relativedelta
from datetime import datetime, timedelta
from pandas.tseries.offsets import Day, MonthEnd

def incrementalSIP(pf_return, tenure, amount, quantSIPinc):
# Number of increments (If there are any kind of increments in monthly investments)
    tenure = tenure + 1
    no_inc = np.nan
    if tenure  % 12 > 0:
        no_inc = tenure // 12 + 1
    else:
        no_inc = tenure // 12

    sip_amt = []
    for multiple in range(no_inc):
        for mth in range(1,12+1):
            sip_amt.append(round(amount*(1 + quantSIPinc)**multiple))
    if tenure % 12 > 0:
        sip_amt = sip_amt[:-(no_inc*12 - tenure)]
    # Correcting the 0th month issue by making the last month SIP as ZERO
    sip_amt[-1] = 0

    # Incremental Principle Amounts
    inc_principle = [amount]
    for i in range(1,tenure):
        inc_principle.append(inc_principle[-1] + sip_amt[i])

    # Incremental Investment Values
    inc_values = [inc_principle[0]]
    for i in range(1,tenure):
        inc_values.append(round(inc_values[-1]*(1+pf_return) + sip_amt[i]))
    values = [inc_principle,inc_values]   
    return values


# Main Function for Retirement Calculations
def retirementCalci(clientAge, clientRetAge, curMthExp, invApproach, principalProtected,
                   preRetireinflation,annuityAnnualRet,lifeExpectency,percentage_SIPinc,currentSavings_Retire):
    # Derived Values
    # Investment Tenure
    invTenure = int(clientRetAge - clientAge)
    # Payout Term
    payoutTerm = int(lifeExpectency - clientRetAge)
    # Inflation to float
    inflation = preRetireinflation/100
    # Error factor
    errorAcceptFactor = int(1000)
    # Annual Percentage in SIP - Converting to float/percentage
    quantumSIPinc = percentage_SIPinc/100
    # Annualizing the Monthly Expenses
    curAnnualExp = int(curMthExp * 12)
    # Future Monthly Expenses at the time of Retirement
    futAnnualExp = int(curAnnualExp*(1 + inflation)**invTenure)
    # Expected Returns during Accumulation Phase - Converting the Annual Returns to Monthly
    annual_inv_return = invApproach / 100
    portfolio_return_mth = (1 + annual_inv_return)**(1/12) - 1
    # Expected Returns Post Retirement - Converting Annual to Monthly
    annuity_annual_return = annuityAnnualRet / 100
    # annuityMonthlyRet = (1 + annuity_annual_return)**(1/12) - 1
    
    
    # Setting up the Left Right and Mid point of the data for Binary Search
    annuity_annual_return = annuityAnnualRet / 100
    principalStart_left = int(futAnnualExp*1/(annuity_annual_return))
    principalStart_right = int(futAnnualExp*1/(annuity_annual_return-(annuity_annual_return-0.0001)))
    principalStart_mid = int((principalStart_right+principalStart_left)/2)
    
    # Dataframe to store yearly calculations
    annuitySchedule = pd.DataFrame(columns = ['Age', 'Annual Payout', 'Actual Return', 'Remaining Amount'])
    age = []
    # Last Year of Retirement Payout is ZERO
    annPayout = [0]
    for i in range(0,payoutTerm+1):
        age.append(int(clientRetAge+i))
        annPayout.append(int(futAnnualExp*(1 + inflation)**(i)))
    # Populating After-Retirement Age and Required Annual Payouts
    annuitySchedule['Age'] = age
    annuitySchedule['Annual Payout'] = annPayout[0:-1]
    
    # Setting up the Left Right and Mid point of the data for Binary Search
    principalStart_left = int(futAnnualExp*1/(annuity_annual_return))
    principalStart_right = int(futAnnualExp*1/(annuity_annual_return-(annuity_annual_return-0.0001)))
    principalStart_mid = int((principalStart_right+principalStart_left)/2)
    # Just a dummy value to start the while loop
    errorFactor_mid = errorAcceptFactor*2
    
    # Loop runs as long as the error factor is above 1000
    while (errorFactor_mid >= errorAcceptFactor):
        principalStart_mid = int((principalStart_right+principalStart_left)/2)
        actRet_left = [0]
        remAmt_left = [principalStart_left]
        actRet_mid = [0]
        remAmt_mid = [principalStart_mid]
        actRet_right = [0]
        remAmt_right = [principalStart_right]
        for j in range(0,payoutTerm):
            actRet_left.append(int(remAmt_left[-1]*annuity_annual_return))
            remAmt_left.append(remAmt_left[-1] + actRet_left[-1] - annuitySchedule['Annual Payout'].iloc[j+1])
            actRet_mid.append(int(remAmt_mid[-1]*annuity_annual_return))
            remAmt_mid.append(remAmt_mid[-1] + actRet_mid[-1] - annuitySchedule['Annual Payout'].iloc[j+1])
            actRet_right.append(int(remAmt_right[-1]*annuity_annual_return))
            remAmt_right.append(remAmt_right[-1] + actRet_right[-1] - annuitySchedule['Annual Payout'].iloc[j+1])
        if principalProtected == 'Yes':
            errorFactor_left = abs(principalStart_left - remAmt_left[-1])
            errorFactor_mid = abs(principalStart_mid - remAmt_mid[-1])
            errorFactor_right = abs(principalStart_right - remAmt_right[-1])
        else:    
            errorFactor_left = abs(remAmt_left[-1])
            errorFactor_mid = abs(remAmt_mid[-1])
            errorFactor_right = abs(remAmt_right[-1])
    # Binary Search method to eleminate data quickly
        if (errorFactor_mid == errorFactor_left) & (errorFactor_mid == errorFactor_left):
            errorFactor_mid = errorAcceptFactor - 1
        elif errorFactor_left == max(errorFactor_left, errorFactor_mid, errorFactor_right):
            principalStart_left = principalStart_mid
        elif errorFactor_right == max(errorFactor_left, errorFactor_mid, errorFactor_right):
            principalStart_right = principalStart_mid
    
    # Populating the desired result in the dataframe
    annuitySchedule = pd.DataFrame({
        'Age': annuitySchedule['Age'].to_list(),
        'Actual Return': list(actRet_mid),
        'Remaining Amount': list(remAmt_mid),
        'Annual Payout': annuitySchedule['Annual Payout'].to_list()
        })

    # Plotting a graph for post retirement Cashflow
    # ----------------------------
    # Retirement Withdrawal Phase
    # ----------------------------
    import plotly.graph_objects as go
    from plotly.subplots import make_subplots
    fig2 = make_subplots(specs=[[{"secondary_y": True}]])

    # Remaining Amount (Left Axis)
    fig2.add_trace(
        go.Bar(
            x=annuitySchedule["Age"].astype(str).tolist(),
            y=annuitySchedule["Remaining Amount"].tolist(),
            name="Remaining Amount",
            hovertemplate="<b>At Age %{x}</b><br>Remaining Amount: ₹%{y:,.0f}<extra></extra>"
        ),
        secondary_y=False
    )

    # Annual Payout (Right Axis)
    fig2.add_trace(
        go.Scatter(
            x=annuitySchedule["Age"].tolist(),
            y=annuitySchedule["Annual Payout"].tolist(),
            name="Annual Payout",
            mode="lines+markers",
            hovertemplate ="<b>At Age %{x}</b><br>Annual Payout: ₹%{y:,.0f}<extra></extra>",
        ),
        secondary_y=True
    )

    fig2.update_layout(
        title="",
        template="plotly_white",
        xaxis_title="Age",
        yaxis_title="",
        autosize=True,
        margin=dict(l=5,r=5,t=5,b=10),
        legend=dict(x=0.01,y=0.99,bgcolor="rgba(0,0,0,0)",borderwidth=0),
        hovermode="x unified",
        yaxis=dict(separatethousands=True)
    )
    fig2.update_layout(
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
        font=dict(color="white"),
        xaxis=dict(gridcolor="#444"),
        yaxis=dict(gridcolor="#444")
    )
    fig2.update_yaxes(secondary_y=False, showline=True,showgrid=False)
    fig2.update_yaxes(secondary_y=True, showline=True,showgrid=False)

    # Force convert numpy arrays to Python lists (prevents binary encoding)
    fig2_dict = fig2.to_dict()

    for trace in fig2_dict["data"]:
        if hasattr(trace["x"], "tolist"):
            trace["x"] = trace["x"].tolist()
        if hasattr(trace["y"], "tolist"):
            trace["y"] = trace["y"].tolist()

    fig2 = go.Figure(fig2_dict)
    
    # What needs to be done to achieve the amount required for Retirement
    # Total Required Amount at the time of Retirement
    amtRequired_retire = int(annuitySchedule['Remaining Amount'].iloc[0])
    
    # Future Value of Current Savings
    value_currentSavings = int(currentSavings_Retire*(1 + portfolio_return_mth)**(invTenure*12-1))
    
    # Lumpsum (One Time) Investment Required today
    investToday = int(
                        (annuitySchedule['Remaining Amount'].iloc[0] - value_currentSavings) /
                        ((1 + annual_inv_return) ** invTenure)
                    )
    # Monthly SIP needed to achieve the target principal amount required on the first yaer of retirement
    if investToday <= 0:
        amountSIP = 0
    else:
        amountSIP = int(npf.pmt(portfolio_return_mth, invTenure*12, 0, 
                                -(annuitySchedule['Remaining Amount'].iloc[0]-value_currentSavings), 0))
    
    # Creating dummy dates based on todays dates (Taking month start)
    initial_date = datetime.today().strftime('%Y-%m-%d')
    
    # Make 02-29 to 02-28 where applicable
    if '02-29' in initial_date:
        initial_date = datetime.strptime(initial_date, '%Y-%m-%d').replace(day=28).strftime('%Y-%m-%d')
            
    # Extracting the dates based on the chosen tenure
    value_dates = [initial_date]
    for month in range(1,invTenure*12+1):
        value_dates.append((datetime.strptime(initial_date, '%Y-%m-%d') +     \
                            relativedelta.relativedelta(months=month)).strftime('%Y-%m-%d'))
    
    # Dataframe to save the investment schedule
    incSIPschedule = pd.DataFrame(index = value_dates, columns = ['SIP Amount', 'SIP Value'])

    if investToday <= 0:
        incSIPschedule['SIP Amount'] = 0
        incSIPschedule['SIP Value'] = 0
    else: 

        # Incremental SIP value - Back Calculation
        period = invTenure*12
        upperSIPlimit = int(npf.pmt(portfolio_return_mth, period, 0, -(annuitySchedule['Remaining Amount'].iloc[0]-value_currentSavings), 0))
        # Setting up the Left Right and Mid point of the data for Binary Search
        sipStart_left = int(1)
        sipStart_right = int(upperSIPlimit)
        sipStart_mid = int((sipStart_left + sipStart_right)/2)
        # Just a dummy value to start the while loop
        sipError_mid = errorAcceptFactor*2
        
        # Loop runs as long as the error factor is above "errorAcceptFactor"
        while (sipError_mid >= errorAcceptFactor):   
            value_left = incrementalSIP(portfolio_return_mth, period, sipStart_left, quantumSIPinc)
            value_right = incrementalSIP(portfolio_return_mth, period, sipStart_right, quantumSIPinc)
            value_mid = incrementalSIP(portfolio_return_mth, period, sipStart_mid, quantumSIPinc)
        # Error Values
            sipError_left = abs(value_left[1][-1] - (annuitySchedule['Remaining Amount'].iloc[0]-value_currentSavings))
            sipError_right = abs(value_right[1][-1] - (annuitySchedule['Remaining Amount'].iloc[0]-value_currentSavings))
            sipError_mid = abs(value_mid[1][-1] - (annuitySchedule['Remaining Amount'].iloc[0]-value_currentSavings))
        # Binary Search method to eleminate data quickly
            if (sipError_mid == sipError_left) & (sipError_mid == sipError_left):
                sipError_mid = errorAcceptFactor - 1
            elif sipError_left == max(sipError_left, sipError_mid, sipError_right):
                sipStart_left = sipStart_mid
            elif sipError_right == max(sipError_left, sipError_mid, sipError_right):
                sipStart_right = sipStart_mid
            sipStart_mid = int((sipStart_left + sipStart_right)/2)
        # Allocating the data to respective fields in the dataframe
        incSIPschedule['SIP Amount'] = value_mid[0]
        incSIPschedule['SIP Value'] = value_mid[1]
    
    # Current Savings Schedule
    savingsSchedule = pd.DataFrame(index = value_dates, columns = ['Savings Amount', 'Savings Value'])
    if currentSavings_Retire > 0:
        savingsSchedule['Savings Amount'] = currentSavings_Retire
        savingsValue = [currentSavings_Retire]
        
        for i in range(len(savingsSchedule.index)-1):
            savingsValue.append(int(savingsValue[i]*(1+portfolio_return_mth)))
    
        savingsSchedule['Savings Value'] = savingsValue
    else:
        savingsSchedule['Savings Amount'] = 0
        savingsSchedule['Savings Value'] = 0
    
    # Investment Journey
    investmentSchedule = pd.concat([incSIPschedule,savingsSchedule], axis=1)
    investmentSchedule['Investment Amount'] = (
            investmentSchedule['SIP Amount'] + investmentSchedule['Savings Amount']
        )
    investmentSchedule['Expected Value'] = (
            investmentSchedule['SIP Value'] + investmentSchedule['Savings Value']
        )
    # FORCE NUMERIC
    investmentSchedule = investmentSchedule.apply(pd.to_numeric, errors='coerce')

    # Investment Schedule for Graph
    invSchedule_graph = pd.DataFrame({
        'Date': investmentSchedule.index.astype(str).to_list(),
        'Investment Amount': investmentSchedule['Investment Amount'].to_list(),
        'Expected Value': investmentSchedule['Expected Value'].to_list()
        })

    # Plotting a graph for pre retirement Investmentflow
    # ----------------------------
    # Retirement Corpus Growth
    # ----------------------------
    
    fig1 = make_subplots(specs=[[{"secondary_y": False}]])

    fig1.add_trace(
        go.Scatter(
            x=invSchedule_graph['Date'].astype(str).tolist(),
            y=invSchedule_graph['Expected Value'].tolist(),
            name="Expected Value",
            mode="lines+markers",
            hovertemplate=
            "<b>Expected Value</b>: ₹%{y:,.0f}" +
            "<extra></extra>"
        )
    )    
    fig1.add_trace(
        go.Scatter(
            x=invSchedule_graph['Date'].astype(str).tolist(),
            y=invSchedule_graph['Investment Amount'].tolist(),
            name="Investment Amount",
            mode="lines+markers",
            hovertemplate=
            "<b>Investment Amount</b>: ₹%{y:,.0f}" +
            "<extra></extra>"
        )
    )
    
    fig1.update_layout(
        title="",
        template="plotly_white",
        xaxis_title="",
        yaxis_title="",
        autosize=True,
        margin=dict(l=5,r=5,t=10,b=5),
        legend=dict(x=0.01,y=0.99,bgcolor="rgba(0,0,0,0)",borderwidth=0),
        hovermode="x unified",
        yaxis=dict(
            separatethousands=True
        )
    )
    fig1.update_layout(
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
        font=dict(color="white"),
        xaxis=dict(gridcolor="#444"),
        yaxis=dict(gridcolor="#444")
    )
    
    fig1.update_xaxes(showline=True, showgrid=False)
    fig1.update_yaxes(showline=True, showgrid=False)

    # Force convert numpy arrays to Python lists (prevents binary encoding)
    fig1_dict = fig1.to_dict()

    for trace in fig1_dict["data"]:
        if hasattr(trace["x"], "tolist"):
            trace["x"] = trace["x"].tolist()
        if hasattr(trace["y"], "tolist"):
            trace["y"] = trace["y"].tolist()

    fig1 = go.Figure(fig1_dict)

    if currentSavings_Retire > 0:
        requirement_message = (
            f"At the Retirement Age, you will need Rs. {amtRequired_retire} \n\n"
            f"and currently you have earmarked Rs. {int(currentSavings_Retire)} for the same."
        )
    else:
        requirement_message = (
            f"At the Retirement Age, you will need Rs. {amtRequired_retire}."
        )
    if investToday <= 0:
        retirement_message = (
            f"Your current savings put you in a strong position for retirement. \n\n"
            f"With our guidance on investment decisions, you’ll remain on track throughout your journey."
        )
    else:
        if (quantumSIPinc == 0) or (invTenure <= 1):
            retirement_message = (
                f"We can help you implement either of the <b>two</b> options to achieve your perfect retirement.<br>"
                f"<i>Option 1. Invest <b>Rs. {investToday}</b> today for the next <b>{invTenure}</b> years.<br></i>"
                f"<i>Option 2. Invest <b>Rs. {amountSIP}</b> monthly for the next <b>{invTenure}</b> years.</i>"
            )
        else:
            retirement_message = (
                f"We can help you implement either of the <b>three</b> options to achieve your perfect retirement.<br>"
                f"<i>Option 1. Invest <b>Rs. {investToday}</b> today for the next <b>{invTenure}</b> years.<br></i>"
                f"<i>Option 2. Invest <b>Rs. {amountSIP}</b> monthly for the next <b>{invTenure}</b> years.<br></i>"
                f"<i>Option 3. Start investing <b>Rs. {incSIPschedule['SIP Amount'].iloc[0]}</b> monthly "
                f"and increase it by <b>{percentage_SIPinc}%</b> annually.</i>"
            )

    return investmentSchedule, annuitySchedule, fig1, fig2, requirement_message, retirement_message