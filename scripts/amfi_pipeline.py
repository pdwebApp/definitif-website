...
def run_pipeline():
    ...
    dates_to_load = amfiNAV['nav_date_str'].unique().tolist()
    if dates_to_load:
        existing = (
    ...
    if existing.data:
        ...
        # Normalize to string date
        existing_df['nav_date'] = pd.to_datetime(existing_df['nav_date']).dt.strftime('%Y-%m-%d')

        # Create a unique key for anti-join
        existing_df['key'] = existing_df['isin'].astype('string').str.strip() + "|" + existing_df['nav_date']
        amfiNAV['key'] = amfiNAV['isin'].astype('string').str.strip() + "|" + amfiNAV['nav_date_str']

        # Anti-join: keep only rows whose key is NOT in existing
        new_keys = set(amfiNAV['key']) - set(existing_df['key'])
        amfiNAV = amfiNAV[amfiNAV['key'].isin(new_keys)].copy()
        amfiNAV = amfiNAV.drop(columns=['key'])

        print(f"After pre-check, {len(amfiNAV)} rows remain to be upserted.")
    ...
    # Drop helper column before upsert
    if 'nav_date_str' in amfiNAV.columns:
        amfiNAV = amfiNAV.rename(columns={'nav_date_str': 'nav_date'})
...
if __name__ == "__main__":
    try:
        ...
        # Catch any unexpected error so the cron job always exits 0
        print(f"Unexpected error in pipeline: {e}")
        print("Marking job as completed so downstream jobs can run.")
        sys.exit(0)